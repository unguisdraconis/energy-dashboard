  ...
  useEffect(() => {
    ...
    const yearData = countries
      .map((country) => {
        ...
        if (!row) return null;
        const total = ENERGY_SOURCES.reduce((sum, src) => sum + (row[src] ?? 0), 0);
      ...
      .filter(Boolean)
      .sort((a, b) => {
        // Sort by fossil share descending
    ...
    g.append('g')
      ...
      .call((g) => g.selectAll('.tick text').attr('fill', '#8892a4').attr('font-size', '10px'));

    // Tooltip
    const tooltipG = g.append('g').style('display', 'none');
    const tooltipRect = tooltipG.append('rect')
      .attr('rx', 4).attr('fill', '#1a2332').attr('stroke', '#4a5568').attr('opacity', 0.95);
    const tooltipText = tooltipG.append('text').attr('fill', '#e2e8f0').attr('font-size', '10px');
    ...
    svg
      ...
      .on('mousemove', function (event) {
        ...
        if (!row) return;
        const src = Object.entries(ENERGY_COLORS).find(([, c]) => c === fill)?.[0];
        if (!src) return;

        tooltipG.style('display', null);
        const label = `${ENERGY_LABELS[src]}: ${row[src].toFixed(1)}%`;
        tooltipText.text(label).attr('x', 6).attr('y', 14);
        const bbox = tooltipText.node().getBBox();
        tooltipRect.attr('width', bbox.width + 12).attr('height', bbox.height + 8);

        const [mx, my] = d3.pointer(event, g.node());
        tooltipG.attr('transform', `translate(${Math.min(mx + 10, w - bbox.width - 15)},${my - 20})`);
      })
